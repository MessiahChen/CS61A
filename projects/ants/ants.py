...
class Bee(Insect):
    ...
    is_watersafe = True
    once_scared = False  # Use once_scared to check whether a scare_ant attacked the bee
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        # New action function implemented in make_scare()
        # END EC
        if self.blocked():
...
class Ant(Insect):
    ...
    blocks_path = True
    is_container = False
    is_doubled = False  # Use is_doubled to check whether the ant's damage is doubled by the queen
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while tmp_place != beehive:
            if tmp_place.bees and self.min_range <= step <= self.max_range:
                return random_or_none(tmp_place.bees)
            else:
                step += 1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        # For byeongmin@berkeley.edu:
        ''' 
        Sorry but I think there is no Ant.reduce_armor method in Ant.
        And since the reduce_armor methods may differ among different types of
        ants, I think it's better to assign a unique reduce_armor method to each
        type of ants.
        '''
        bees_tmp = self.place.bees[:]
    ...
    def action(self, colony):
        ...
        if self.imposter == True:
            ...
            while place.exit != None:
                place = place.exit
                if place.ant:
                    # Check whether the ant's damage is doubled
                    if not place.ant.is_doubled:
                        place.ant.damage *= 2
                        place.ant.is_doubled = True
                    # Check whether the ant is a container and the contained ant's damage is doubled
                    if place.ant.is_container and place.ant.contained_ant and \
                            not place.ant.contained_ant.is_doubled:
