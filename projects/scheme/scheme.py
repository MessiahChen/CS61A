...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        return True
    elif expressions.rest == nil and scheme_truep(expressions.first):
        return scheme_eval(expressions.first, env, True)
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    "*** YOUR CODE HERE ***"
    formals, exprs = nil, nil
    while bindings != nil:
        check_form(bindings.first, 2, 2)
        formals = Pair(bindings.first.first, formals)
        exprs = Pair(scheme_eval(bindings.first.rest.first, env), exprs)
        bindings = bindings.rest
    check_formals(formals)
    return env.make_child_frame(formals, exprs)
    # END PROBLEM 14
...
def do_define_macro(expressions, env):
    ...
    func, formals = target.first, target.rest
    body = expressions.rest

    env.define(func, MacroProcedure(formals, body, env))
